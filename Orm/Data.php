...
class Data
{
	...
	/* DEBUG */
	public static $var_dump_stack = array();
	public function var_dump()
	{
		$data = $this->data;
		
		// Loops through all other possible columns and 'get' them
		foreach(array_merge(
			array_keys((array) $this->model['one-to-one']),
			array_keys((array) $this->model['many-to-one']),
			array_keys((array) $this->model['one-to-many']),
			array_keys((array) $this->model['many-to-many'])
		) as $key) {
