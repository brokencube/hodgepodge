...
class Model implements \JsonSerializable
{
    ...
    /* Quick and dirty functions to parse out the 'model' objects in this object so that var_dump display is not humungous. */
    public static $debug_url = '/debug/';

    public function var_dump()
    {
        $data['template']['url'] = self::$debug_url;
        $data['template']['root'] = 'file:'.LIB_ROOT.'/templates/model_dump.tpl';
        $data['template']['model'] = 'file:'.LIB_ROOT.'/templates/model_dump_model.tpl';
        $data['template']['array'] = 'file:'.LIB_ROOT.'/templates/model_dump_array.tpl';
        $data['class'] = $this->table;
        $data['id'] = $this->id;
        
        echo Core\SmartyTemplate::render($data['template']['root'], $data);
    ...
    public static function debug()
    {
        ...
        while ($args)
        {
            ...
            $id = array_shift($args);
            $breadcrumb .= "$class/$id/";
            $object = Model::factoryObjectCache($id, $class, static::$dbconnection, true);
          
            $list[str_repeat(' ', $counter++) . get_class($object) . '(' . $object->table . "):$id"] = $breadcrumb;
        }
        
        array_pop($list);
        
        $obj = Model::factoryObjectCache($id, $class, static::$dbconnection, true);
        
        if ($obj)
        {
            $data['breadcrumb'] = $list;
            $data['class'] = get_class($obj) . '(' . $obj->table . ')';
            $data['id'] = $obj->id;
            
            $data['columns'] = get_object_vars($obj);
            unset($data['columns']['_db']);
            unset($data['columns']['model']);
            ...
            echo Core\SmartyTemplate::render($data['template']['model'], $data);
        }
        else
        {
