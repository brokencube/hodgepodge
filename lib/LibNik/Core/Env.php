...
class Env
{
    ...
    protected $host_os = null;               // OS the server is running: windows or linux
    protected $host_name = null;             // Actual name of the host machine, not the HTTP host
    protected $http_host = null;             // HTTP Host (i.e. the vhost ServerName or Apache ServerName, which can vary)
    protected $php_mode = null;              // How PHP was invoked: apache or cli
    protected $override_file = null;         // Name of config override file loaded, if any
    ...
    protected function __construct() 
    {
        ...
        if (preg_match('/^Linux/', php_uname())) {
            $this->host_os = 'linux';
        } elseif (preg_match('/^Windows/', php_uname())) {
            $this->host_os = 'windows';
        }
        
        // Determine host name
        if ($this->host_os == 'linux') {
            $uname = php_uname('n');
            if (strlen($uname)) {
